...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-granite-lang-bytecode-detail.ps1

julia --project=. tests/domain_review.jl

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-granite-lang-bytecode-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-granite-lang-bytecode-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-granite_lang_bytecode_probe-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
